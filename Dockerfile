...
RUN |
  chmod uga+x $INST_SCRIPTS/firefox.sh
  $INST_SCRIPTS/firefox.sh

ADD src/install/chrome.sh .
RUN |
  chmod uga+x $INST_SCRIPTS/chrome.sh
...
RUN |
  chmod uga+x ./set_user_permission.sh
